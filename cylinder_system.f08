...
subroutine half_leap(thermostat)!the verlet integrator requires to update the velocities in two steps, the subroutine distinguishes when the particles have the same mass and when they don't
	...
	real(8)::dran_g!gaussian random number
	real(8)::dran_u!uniform random number
	real(8)::threshold=tstep/trelax!this variable is used to test if a particle will be affected by the andersen thermostat
	do k=1,numcells(3)
		do j=1,numcells(2)
			do i=1,numcells(1)
				do n=1,occup(i,j,k)
...
subroutine calculate_forces
	...
	fvec=0.d0!setting forces to zero
	do k1=1,numcells(3)
		do j1=1,numcells(2)
			do i1=1,numcells(1)
...
subroutine radial_distribution
	...
	integer(8)::distbins(20)=0!these bins will count how many particles are in the divisions of the (0,L/2] interval in which the distribution will be measured
	integer(8)::i=0!particle counter
	integer(8)::tag1=0!tag for the first particle
	integer(8)::tag2=0!tag for the second particle
	real(8)::dist=1.d10!distance between particles
	real(8)::l=dmin1(siz(1),siz(2),siz(3))*5.d-1!this is the L factor of the above interval, chosen relatively to the smallest side of the simulation box
	real(8)::temp=0.d0!temporary storage for the recursive search of the minimum distance to the centre of the simulation box
	do i=1,amnttot
		temp=dsqrt((rvec(1,i)-siz(1)*5.d-1)**2+(rvec(2,i)-siz(2)*5.d-1)**2+(rvec(3,i)-siz(3)*5.d-1)**2)!distance between the particle i and the centre of the simulation box
		if(temp<dist)then
			dist=temp!here the minimum is found recursively
			tag=i!and the tag is kept for later use
		end if
	end do
	do i=1,amnttot
		dist=dsqrt((rvec(1,tag)-rvec(1,i))**2+(rvec(2,tag)-rvec(2,i))**2+(rvec(3,tag)-rvec(3,i))**2)
		if(dist<=l)then
			bintag=nint(dist*2.d1/l,8)
			distbins(bintag)=distbins(bintag)+1
		end if
	end do
	return
